...
bool bench_find_next_states(){
    ...
    return state.current_polygon.size_ll == 0;
}

TEST_CASE("bench_find_next_states", "[Number vs Float]"){

    BENCHMARK("find_next_states Float<double>"){
        return bench_find_next_states<Float<double>>();
    };
    BENCHMARK("find_next_states mpz_class"){
        return bench_find_next_states<Number<mpz_class, 2, 5, 13, 17>>();
    };
}
